...
const NewsCart = ({ info }) => {
  ...
  return (
    <div
      className="flex flex-col  justify-between items-start h-fit mx-9"
      style={{ width: "20rem" }}
    >
      <div style={{ width: "100%", height: "18rem", position: "relative" }}>
        ...
        <div
          style={{
            ...
            background: "#fff",
            borderRadius: "0.5rem",
            boxShadow: "0px 0px 10px 0px #DCF5FA",
          }}
        >
      ...
      <div className="flex flex-row justify-between items-center  w-1/3">
        ...
        >{`${new Date(info.creation_date).getFullYear()} - ${new Date(
          info.creation_date
        ).getMonth()} - ${new Date(info.creation_date).getDate()}`}</p>
        <p
          style={{ color: "#D184FB" }}
          className=" font-normal text-base"
        >{`${new Date(info.creation_date).getHours()} :  ${new Date(
          info.creation_date
        ).getMinutes()} `}</p>
      </div>
      <div
        style={{ color: "#3B0359" }}
        className=" font-normal text-lg overflow-hidden h-12"
      >
